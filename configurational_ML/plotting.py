...
def plot_configuration(configuration, energy=None, same=False):
    ...
    if same:
        ...
        ax.imshow(np.ma.masked_array(configuration, configuration),)

        legend_elements = [Patch(facecolor='#ff0000',
                                 label='S1'),
...
def show_cell_occ(occ, spin_channel= 0):
    ...
    for i in range(z):
        inner = gridspec.GridSpecFromSubplotSpec(x, y,
        ...
        for j in range(x):
            for k in range(y):
                ...
                if np.any(np.abs(occ[j,k,layer]) > 0.5, axis=(-1,-2,-3)):
                    ax.imshow(occ[j,k,layer,spin_channel,:,:],
                              vmin=np.min(occ),
                              vmax=np.max(occ),
                              cmap='plasma')
                else:
