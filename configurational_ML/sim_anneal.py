...
def move(configuration, restrict_ov=False, restrict_pol=False, random=0.0):
    ...
    if np.random.rand() < random:
        masks = Descriptor(configuration, (int(configuration.shape[0]/2-1),
                                           int(configuration.shape[1]/2-1))).mask
...
def predict(configurations, model, weights, scaler, radius=(6,4), iterations=8, alpha=0.1):
    ...
    return model(descriptors,
                 ...
                 weights[0],
                 weights[1],
                 weights[2]).reshape(-1,4).mean(axis=1)
...
def sim_anneal(configuration, weights, desc_scaler, seed=1234, temps=np.ones(5)*0.01, restrict_ov = False, restrict_pol = False, random=0.01, print_freq=100):
    ...
    energy = predict(configuration[np.newaxis], model, weights, desc_scaler, radius=desc_size)
    ...
    for i,temp in enumerate(temps):
        ...
        new_configurations = move(configuration, 
                restrict_ov = restrict_ov, 
                restrict_pol = restrict_pol, 
                random=random)
        new_energies = predict(new_configurations, model, weights, desc_scaler, radius=desc_size)
