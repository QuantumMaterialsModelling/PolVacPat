...
def extract_data(path, cell, partitions=(12,8)):
    configurations = []
    energies = []
    ...
    for x,y,mag,defect in zip(data['pos'], 
                                data['en'], 
                                data['mag'], 
                                data['defect']):
...
def convert_to_desc(configurations, 
                    energies, 
                    desc_scaling=None, 
                    en_scaling=None,
                    size=(6,4),
    ...
    for i,configuration in enumerate(configurations):
        ...
        if rescale_factor != 1:
            d, idxs, c = np.unique(feature, return_index=True, return_counts=True,axis=0,)
...
class Descriptor:
    def __init__(self, configuration, width):
        ...
        for i, (w, s) in enumerate(zip(self.width, self.shape_init)):
            if s < 2*w+1:
                self.config = np.tile(self.config, 
    ...
    def diffuse(self, config, iterations, alpha):
        ...
        for _ in range(iterations):
            diffused += np.moveaxis([convolve2d(diffused[:,:,i], 
                                                alpha*kernel, 
                                                mode='same', 
                                                boundary='wrap') for i in range(3)], 0, 2)
...
class Cell_grid_interface():
    ...
    def __init__(self, cell: np.array, partitions: tuple, value_shape:tuple=()):
        self.cell = cell
        self.grid = np.zeros(partitions+value_shape)
        self.bins = [np.linspace(0, 1, partitions[0]+1),
                     np.linspace(0, 1, partitions[1]+1),
                     np.linspace(0, 1, partitions[2]+1,)]
        self.dx = np.array([[tmp[1]-tmp[0] for tmp in self.bins]])
        self.indices = None

    def get_indices(self, positions, shift=True, frac=0.0):
        ...
        shift : bool, default=True
            ...
            their origin. Might be needed for allignment purposes.
    ...
    def fill_grid(self, values, sites=(0, -1)):
        ...
        to each site are determined and with fill_grid the correspodning
        magnetizations can be assigned to the grid cells.

        Parameters
        ----------
        values : array
            values the should be assgined to the indices.
